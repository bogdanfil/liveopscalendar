...
The Planning windows group includes Planning year and 6, 8, or 10 full months starting two months before the current month. The remaining 4, 6, or 8 months include the current month. An additional 6-month window (1 before / 5 after) is selected by default. It starts with the previous calendar month and includes the current month plus four following months. Only Production is enabled on page opening. The Development eye toggle starts off and switches current, late, planned, and estimated development together. For example, in September the 6-month view runs July–December, the 8-month view July–February, and the 10-month view July–April.
...
Columns are matched by their headers: Month (also accepts Calendar), Event, Feature, Development Team, Track, Prod Dates, Dev Dates, and comment. Missing required headings are listed in the loading error. Features sharing a Track share one Schedule row; production occupies the upper lanes and development is always placed below it. Overlapping bars of the same phase use additional lanes within that row. Features with no Track get their own row.

The calendar has a single Schedule view. It shows production windows, with yellow bars for features currently in development. Development includes transparent windows active today, unfinished development whose end date has passed or whose sheet cell is marked red, future planned dates entered in the sheet, and inferred estimates. Completed manual development is excluded. The single Development eye toggle controls all four development layers together and starts off. When development dates are missing, the estimate starts 30 calendar days before the first production date and lasts 14 days, counting the start day. If production dates are also missing, the first day of the planned month remains the fallback anchor. Estimates remain clearly flagged and never write dates back to the sheet.

Development bars omit the DEV prefix. Both development and production bars place their status icon at the far right inside the box, only when it fits alongside the full label. Spacing tightens on narrow bars to keep the icon visible when there is room. Overdue development clocks move just outside the right edge when they cannot fit inside. Warning exclamations remain outside the right edge, with space reserved between neighbouring bars and at the end of the date range. Status icons show planned (clock), in progress (gear), done (check), and late (red clock with an exclamation badge) or missing dates (exclamation). Missing production dates appear as dashed placeholders in their planned month. Red exclamations identify date and status warnings. Amber split arrows with two separate upward arrowheads identify overlapping team development, independently of other warnings. Both icons appear when a feature has both kinds of alert, including in hover details and the alerts panel. Hover or focus a bar for its status, description, and warnings; click for full details and a link to its exact source cell.
